...
不同 GitHub 链接会复用同一连接，减少重复建连；支持 HTTP/2 时，文件预览的分支和标签查询可以并发进行。
首次建连和每次 API 读取仍需要网络时间。成功的预览在内存中保留最多 60 秒，重复悬停可更快显示；同一链接同时发起的
请求会合并为一次读取。因此，状态和 Milestone 进度可能延迟最多一分钟。每次悬停仍会检查本机当前
`gh` 凭据，切换账号或退出登录后清除旧内容。服务端权限变化会在下一次 API 读取时确认，访问被拒绝后
会清除已保留的内容。

预览不即时生成 AI 摘要，不向 AI 服务发送内容，也不把 GitHub 预览内容写入磁盘缓存。关闭本地服务即
清除内存缓存。其他网站和 GitHub Enterprise 域名暂不在支持范围内。
