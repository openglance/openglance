...
export async function createGithubSmokeFixture(root, { http2 = true } = {}) {
  ...
  let connections = 0, activeRefs = 0, maxActiveRefs = 0;
  const api = (http2 ? createHttp2Server : createHttpsServer)({ key: readFileSync(key), cert: readFileSync(cert), allowHTTP1: true }, async (request, response) => {
    const endpoint = request.url, token = request.headers.authorization;
    requests.push({ endpoint, token, protocol: request.httpVersion });
    ...
    const ref = endpoint.includes("/git/matching-refs/");
    if (ref) maxActiveRefs = Math.max(maxActiveRefs, ++activeRefs);
    await new Promise((resolve) => setTimeout(resolve, ref ? 300 : slow ? 2200 : endpoint.endsWith("/42") ? 1300 : 80));
    if (ref) activeRefs--;
    const file = endpoint.match(/\/contents\/docs\/([abc])\.md\?ref=/)?.[1];
    const data = ref ? (endpoint.includes("/heads/") ? [{ ref: "refs/heads/main", object: { sha: "b".repeat(40) } }] : [])
      : file ? { type: "file", encoding: "base64", size: 50, content: Buffer.from(`# Remote file ${file}\n\nUncached file ${file} content.`).toString("base64") }
  ...
  // Keep the H1 fixture alive through the smoke's idle gap (Node otherwise advertises five seconds).
  api.keepAliveTimeout = 60000;
  ...
  return {
    ...
    maxConcurrentRefs: () => maxActiveRefs,
